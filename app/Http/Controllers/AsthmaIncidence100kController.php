...
use App\Exports\AsthmaIncidence100kExport;

class AsthmaIncidence100kController extends Controller
{
    public function index(Request $request)
    {
        $table = 'health_as_incidence_100k_all';
        ...
        $summaryRow = (clone $baseQuery)
            ->selectRaw('
                ...
                COALESCE(SUM(patient_as_total),0) as `case`,
        ...
        return view('health.as_incidence_100k', compact(
    ...
    public function export(Request $request)
    {
        ...
        $filename = 'อัตราป่วยรายใหม่โรคหืด';
        if ($year) $filename .= '_' . $year;
        if ($district) $filename .= '_' . $district;
        $filename .= '.xlsx';

        return Excel::download(
            new AsthmaIncidence100kExport($year, $district),
            $filename
        );
    }
}
...
<table border="1" style="border-collapse:collapse; width:100%;">
    ...
    <tbody>
        @forelse($rows as $index => $row)
            <tr>
                ...
                <td style="padding:6px; text-align:right; background:{{ $index % 2 == 0 ? '#edf4fb' : '#e8f1fb' }};">
                    {{ number_format($row->patient_copd_total ?? 0) }}
                </td>
                <td style="padding:6px; text-align:right; background:{{ $index % 2 == 0 ? '#edf4fb' : '#e8f1fb' }};">
                    {{ number_format($row->death_copd_total ?? 0) }}
                </td>
                <td style="padding:6px; text-align:right; background:{{ $index % 2 == 0 ? '#edf4fb' : '#e8f1fb' }};">
                    {{ number_format((float) ($row->percentage_total ?? 0), 2) }}
                </td>

                <td style="padding:6px; text-align:right;">{{ number_format($row->patient_copd_total1 ?? 0) }}</td>
                <td style="padding:6px; text-align:right;">{{ number_format($row->death_copd_total1 ?? 0) }}</td>
                <td style="padding:6px; text-align:right;">{{ number_format((float) ($row->percentage_total1 ?? 0), 2) }}</td>

                <td style="padding:6px; text-align:right;">{{ number_format($row->patient_copd_total2 ?? 0) }}</td>
                <td style="padding:6px; text-align:right;">{{ number_format($row->death_copd_total2 ?? 0) }}</td>
                <td style="padding:6px; text-align:right;">{{ number_format((float) ($row->percentage_total2 ?? 0), 2) }}</td>

                <td style="padding:6px; text-align:right;">{{ number_format($row->patient_copd_total3 ?? 0) }}</td>
                <td style="padding:6px; text-align:right;">{{ number_format($row->death_copd_total3 ?? 0) }}</td>
                <td style="padding:6px; text-align:right;">{{ number_format((float) ($row->percentage_total3 ?? 0), 2) }}</td>

                <td style="padding:6px; text-align:right;">{{ number_format($row->patient_copd_total4 ?? 0) }}</td>
                <td style="padding:6px; text-align:right;">{{ number_format($row->death_copd_total4 ?? 0) }}</td>
                <td style="padding:6px; text-align:right;">{{ number_format((float) ($row->percentage_total4 ?? 0), 2) }}</td>

                <td style="padding:6px; text-align:right;">{{ number_format($row->patient_copd_total5 ?? 0) }}</td>
                <td style="padding:6px; text-align:right;">{{ number_format($row->death_copd_total5 ?? 0) }}</td>
        ...
        <tr>
            ...
            <td style="padding:7px; text-align:right; font-weight:bold; background:#eaf2fb;">{{ number_format($summary->patient_copd_total_sum ?? 0) }}</td>
            <td style="padding:7px; text-align:right; font-weight:bold; background:#eaf2fb;">{{ number_format($summary->death_copd_total_sum ?? 0) }}</td>

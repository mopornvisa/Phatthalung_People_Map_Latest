...
<table border="1" style="border-collapse:collapse; width:100%;">
    ...
    <tbody>
        @forelse($rows as $index => $row)
            <tr>
                ...
                <td style="padding:6px; text-align:right; background:{{ $index % 2 == 0 ? '#edf4fb' : '#e8f1fb' }};">
                    {{ number_format($row->patient_dm_total ?? 0) }}
                </td>
                <td style="padding:6px; text-align:right; background:{{ $index % 2 == 0 ? '#edf4fb' : '#e8f1fb' }};">
                    {{ number_format($row->death_dm_total ?? 0) }}
                </td>
                <td style="padding:6px; text-align:right; background:{{ $index % 2 == 0 ? '#edf4fb' : '#e8f1fb' }};">
                    {{ number_format((float) ($row->percentage_total ?? 0), 2) }}
                </td>

                <td style="padding:6px; text-align:right;">{{ number_format($row->patient_dm_total1 ?? 0) }}</td>
                <td style="padding:6px; text-align:right;">{{ number_format($row->death_dm_total1 ?? 0) }}</td>
                <td style="padding:6px; text-align:right;">{{ number_format((float) ($row->percentage_total1 ?? 0), 2) }}</td>

                <td style="padding:6px; text-align:right;">{{ number_format($row->patient_dm_total2 ?? 0) }}</td>
                <td style="padding:6px; text-align:right;">{{ number_format($row->death_dm_total2 ?? 0) }}</td>
                <td style="padding:6px; text-align:right;">{{ number_format((float) ($row->percentage_total2 ?? 0), 2) }}</td>

                <td style="padding:6px; text-align:right;">{{ number_format($row->patient_dm_total3 ?? 0) }}</td>
                <td style="padding:6px; text-align:right;">{{ number_format($row->death_dm_total3 ?? 0) }}</td>
                <td style="padding:6px; text-align:right;">{{ number_format((float) ($row->percentage_total3 ?? 0), 2) }}</td>

                <td style="padding:6px; text-align:right;">{{ number_format($row->patient_dm_total4 ?? 0) }}</td>
                <td style="padding:6px; text-align:right;">{{ number_format($row->death_dm_total4 ?? 0) }}</td>
                <td style="padding:6px; text-align:right;">{{ number_format((float) ($row->percentage_total4 ?? 0), 2) }}</td>

                <td style="padding:6px; text-align:right;">{{ number_format($row->patient_dm_total5 ?? 0) }}</td>
                <td style="padding:6px; text-align:right;">{{ number_format($row->death_dm_total5 ?? 0) }}</td>
        ...
        <tr>
            ...
            <td style="padding:7px; text-align:right; font-weight:bold; background:#eaf2fb;">{{ number_format($summary->patient_dm_total_sum ?? 0) }}</td>
            <td style="padding:7px; text-align:right; font-weight:bold; background:#eaf2fb;">{{ number_format($summary->death_dm_total_sum ?? 0) }}</td>
        ...
        <tr>
            <td colspan="19" style="padding:8px 10px; background:#f8fbfd; font-size:13px; color:#334155;">
                <strong>B</strong> หมายถึง จำนวนผู้ป่วยด้วยโรคเบาหวานทั้งหมด
            </td>
        </tr>
    </tbody>
</table>

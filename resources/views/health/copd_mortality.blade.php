...
<head>
    ...
    <title>อัตราการป่วยตายด้วยโรคปอดอุดกั้นเรื้อรัง</title>
...
@php
    ...
    $sumPatient1 = $chartRows->sum(fn($r) => (float) ($r->patient_copd_total1 ?? 0));
    $sumDeath1   = $chartRows->sum(fn($r) => (float) ($r->death_copd_total1 ?? 0));
    $sumRate1    = $sumPatient1 > 0 ? ($sumDeath1 / $sumPatient1) * 100 : 0;

    $sumPatient2 = $chartRows->sum(fn($r) => (float) ($r->patient_copd_total2 ?? 0));
    $sumDeath2   = $chartRows->sum(fn($r) => (float) ($r->death_copd_total2 ?? 0));
    $sumRate2    = $sumPatient2 > 0 ? ($sumDeath2 / $sumPatient2) * 100 : 0;

    $sumPatient3 = $chartRows->sum(fn($r) => (float) ($r->patient_copd_total3 ?? 0));
    $sumDeath3   = $chartRows->sum(fn($r) => (float) ($r->death_copd_total3 ?? 0));
    $sumRate3    = $sumPatient3 > 0 ? ($sumDeath3 / $sumPatient3) * 100 : 0;

    $sumPatient4 = $chartRows->sum(fn($r) => (float) ($r->patient_copd_total4 ?? 0));
    $sumDeath4   = $chartRows->sum(fn($r) => (float) ($r->death_copd_total4 ?? 0));
    $sumRate4    = $sumPatient4 > 0 ? ($sumDeath4 / $sumPatient4) * 100 : 0;

    $sumPatient5 = $chartRows->sum(fn($r) => (float) ($r->patient_copd_total5 ?? 0));
    $sumDeath5   = $chartRows->sum(fn($r) => (float) ($r->death_copd_total5 ?? 0));
    ...
    <div class="ga-page shadow-soft">
        ...
        <div class="ga-topbar">
            <div class="ga-title">
                ...
                <span>อัตราการป่วยตายด้วยโรคปอดอุดกั้นเรื้อรัง</span>
            </div>
            ...
            <form method="GET" id="filterForm">
                <div class="row g-3 align-items-end">
                 ...
                 <div class="ga-filter-actions">
                    ...
                    <a href="{{ route('health.copd-mortality.export', request()->query()) }}" class="btn ga-btn-export">
        ...
        <div class="row g-3 mb-3">
            <div class="col-md-4">
                <div class="ga-kpi">
                    ...
                    <h3 class="ga-kpi-value">{{ number_format($summary->patient_copd_total_sum ?? 0) }}</h3>
                </div>
            </div>
            <div class="col-md-4">
                <div class="ga-kpi">
                    <div class="ga-kpi-label">ผู้เสียชีวิตรวม</div>
                    <h3 class="ga-kpi-value">{{ number_format($summary->death_copd_total_sum ?? 0) }}</h3>
                </div>
            </div>
            <div class="col-md-4">
                <div class="ga-kpi">
                    ...
                    <tbody>
                        @forelse($rows as $row)
                            <tr>
                                ...
                                <td class="text-end metric-blue">{{ number_format($row->patient_copd_total) }}</td>
                                <td class="text-end metric-blue">{{ number_format($row->death_copd_total) }}</td>
                                <td class="text-end metric-blue">{{ number_format($row->percentage_total, 2) }}</td>

                                <td class="text-end">{{ number_format($row->patient_copd_total1) }}</td>
                                <td class="text-end">{{ number_format($row->death_copd_total1) }}</td>
                                <td class="text-end">{{ number_format($row->percentage_total1, 2) }}</td>

                                <td class="text-end">{{ number_format($row->patient_copd_total2) }}</td>
                                <td class="text-end">{{ number_format($row->death_copd_total2) }}</td>
                                <td class="text-end">{{ number_format($row->percentage_total2, 2) }}</td>

                                <td class="text-end">{{ number_format($row->patient_copd_total3) }}</td>
                                <td class="text-end">{{ number_format($row->death_copd_total3) }}</td>
                                <td class="text-end">{{ number_format($row->percentage_total3, 2) }}</td>

                                <td class="text-end">{{ number_format($row->patient_copd_total4) }}</td>
                                <td class="text-end">{{ number_format($row->death_copd_total4) }}</td>
                                <td class="text-end">{{ number_format($row->percentage_total4, 2) }}</td>

                                <td class="text-end">{{ number_format($row->patient_copd_total5) }}</td>
                                <td class="text-end">{{ number_format($row->death_copd_total5) }}</td>
                        ...
                        <tr class="ga-total-row">
                            ...
                            <td class="text-end">{{ number_format($summary->patient_copd_total_sum ?? 0) }}</td>
                            <td class="text-end">{{ number_format($summary->death_copd_total_sum ?? 0) }}</td>
        ...
        <div class="ga-legend-box">
            ...
            <div class="ga-legend-item">
                <strong>A</strong> หมายถึง จำนวนผู้ป่วยด้วยโรคปอดอุดกั้นเรื้อรังเสียชีวิต
            </div>
            <div class="ga-legend-item mb-0">
                <strong>B</strong> หมายถึง จำนวนป่วยด้วยโรคปอดอุดกั้นเรื้อรังทั้งหมด
            </div>
        </div>
    </div>
</div>

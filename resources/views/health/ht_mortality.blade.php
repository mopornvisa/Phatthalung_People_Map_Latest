...
<head>
    ...
    <title>อัตราการป่วยตายด้วยโรคความดันโลหิตสูง</title>
    ...
    <style>
        ...
        .ga-filter-actions{
            margin-top:16px; padding-top:16px; border-top:1px dashed #d8e8ef;
        ...
        .ga-legend-box{ margin-top:18px; border-radius:18px; padding:14px 16px; }
        .ga-legend-title{ font-size:13px; font-weight:700; color:#0f172a; margin-bottom:10px; }
        .ga-legend-item{
            font-size:12px; color:#475569; padding:8px 12px; background:#f8fbfd;
            border:1px solid #e5edf4; border-radius:12px; margin-bottom:8px;
        }
        .loading-overlay{
...
@php
    ...
    $sumPatient1 = $chartRows->sum(fn($r) => (float) ($r->patient_ht_total1 ?? 0));
    $sumDeath1   = $chartRows->sum(fn($r) => (float) ($r->death_ht_total1 ?? 0));
    $sumRate1    = $sumPatient1 > 0 ? ($sumDeath1 / $sumPatient1) * 100 : 0;

    $sumPatient2 = $chartRows->sum(fn($r) => (float) ($r->patient_ht_total2 ?? 0));
    $sumDeath2   = $chartRows->sum(fn($r) => (float) ($r->death_ht_total2 ?? 0));
    $sumRate2    = $sumPatient2 > 0 ? ($sumDeath2 / $sumPatient2) * 100 : 0;

    $sumPatient3 = $chartRows->sum(fn($r) => (float) ($r->patient_ht_total3 ?? 0));
    $sumDeath3   = $chartRows->sum(fn($r) => (float) ($r->death_ht_total3 ?? 0));
    $sumRate3    = $sumPatient3 > 0 ? ($sumDeath3 / $sumPatient3) * 100 : 0;

    $sumPatient4 = $chartRows->sum(fn($r) => (float) ($r->patient_ht_total4 ?? 0));
    $sumDeath4   = $chartRows->sum(fn($r) => (float) ($r->death_ht_total4 ?? 0));
    $sumRate4    = $sumPatient4 > 0 ? ($sumDeath4 / $sumPatient4) * 100 : 0;

    $sumPatient5 = $chartRows->sum(fn($r) => (float) ($r->patient_ht_total5 ?? 0));
    $sumDeath5   = $chartRows->sum(fn($r) => (float) ($r->death_ht_total5 ?? 0));
    ...
    <div class="ga-page shadow-soft">
        ...
        <div class="ga-topbar">
            <div class="ga-title">
                ...
                <span>อัตราการป่วยตายด้วยโรคความดันโลหิตสูง</span>
            </div>
            ...
            <form method="GET" id="filterForm">
                <div class="row g-3 align-items-end">
                 ...
                 <div class="ga-filter-actions">
                    ...
                    <a href="{{ route('health.ht-mortality.export', request()->query()) }}" class="btn ga-btn-export">
        ...
        <div class="row g-3 mb-3">
            <div class="col-md-4">
                <div class="ga-kpi">
                    ...
                    <h3 class="ga-kpi-value">{{ number_format($summary->patient_ht_total_sum ?? 0) }}</h3>
                </div>
            </div>
            <div class="col-md-4">
                <div class="ga-kpi">
                    <div class="ga-kpi-label">ผู้เสียชีวิตรวม</div>
                    <h3 class="ga-kpi-value">{{ number_format($summary->death_ht_total_sum ?? 0) }}</h3>
                </div>
            </div>
            <div class="col-md-4">
                <div class="ga-kpi">
                    ...
                    <tbody>
                        @forelse($rows as $row)
                            <tr>
                                ...
                                <td class="text-end metric-blue">{{ number_format($row->patient_ht_total) }}</td>
                                <td class="text-end metric-blue">{{ number_format($row->death_ht_total) }}</td>
                                <td class="text-end metric-blue">{{ number_format($row->percentage_total, 2) }}</td>

                                <td class="text-end">{{ number_format($row->patient_ht_total1) }}</td>
                                <td class="text-end">{{ number_format($row->death_ht_total1) }}</td>
                                <td class="text-end">{{ number_format($row->percentage_total1, 2) }}</td>

                                <td class="text-end">{{ number_format($row->patient_ht_total2) }}</td>
                                <td class="text-end">{{ number_format($row->death_ht_total2) }}</td>
                                <td class="text-end">{{ number_format($row->percentage_total2, 2) }}</td>

                                <td class="text-end">{{ number_format($row->patient_ht_total3) }}</td>
                                <td class="text-end">{{ number_format($row->death_ht_total3) }}</td>
                                <td class="text-end">{{ number_format($row->percentage_total3, 2) }}</td>

                                <td class="text-end">{{ number_format($row->patient_ht_total4) }}</td>
                                <td class="text-end">{{ number_format($row->death_ht_total4) }}</td>
                                <td class="text-end">{{ number_format($row->percentage_total4, 2) }}</td>

                                <td class="text-end">{{ number_format($row->patient_ht_total5) }}</td>
                                <td class="text-end">{{ number_format($row->death_ht_total5) }}</td>
        ...
        <div class="ga-legend-box">
            ...
            <div class="ga-legend-item">
                <strong>A</strong> หมายถึง จำนวนผู้ป่วยด้วยโรคความดันโลหิตสูงเสียชีวิต
            </div>
            <div class="ga-legend-item mb-0">
                <strong>B</strong> หมายถึง จำนวนผู้ป่วยด้วยโรคความดันโลหิตสูงเสียชีวิต
            </div>
        </div>
    </div>
</div>
